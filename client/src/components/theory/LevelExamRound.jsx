...
// Unlike TheoryDrillRound, this doesn't reveal correct/incorrect per
// question -- it's a certification exam, not a practice round. Score
// and pass/fail only appear at the end, and the score itself is
// server-computed (POST /api/exam/attempts re-checks every answer
// against the DB, not the client's own tally).
export default function LevelExamRound({ language, level, onExit }) {
  ...
  const { recordCard, endNow } = useStudySession('level_exam', language);

  useEffect(() => {
    api
      .getExamQuestions(language, level)
  ...
  async function next() {
    const updatedAnswers = [...answers, { drill_id: questions[index].id, selected }];
    setAnswers(updatedAnswers);

    if (index + 1 >= questions.length) {
      setSubmitting(true);
      try {
        const res = await api.submitExamAttempt(language, level, updatedAnswers);
  ...
  if (result) {
    return (
      <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-6 text-center space-y-3">
        <div className={`text-2xl ${result.passed ? '' : 'grayscale'}`}>{result.passed ? '🏆' : '📚'}</div>
        <div className="text-lg font-semibold">
          {result.score} / {result.total}
        </div>
        <div className={`text-sm ${result.passed ? 'text-emerald-400' : 'text-neutral-400'}`}>
          {result.passed ? `Уровень ${level} сдан!` : 'Нужно 100% правильных ответов — попробуй ещё раз.'}
        </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="text-xs text-neutral-500 text-center">
        Вопрос {index + 1} из {questions.length} · {language === 'kz' ? 'Казахский' : 'English'} · {level}
      </div>
