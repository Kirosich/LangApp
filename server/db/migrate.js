...
// ru->lang typing (production direction: shown Russian, type the
// target-language word) tracked as its own session_type so accuracy
// there doesn't blend with the existing lang->ru direction -- they test
// different skills (recall vs. production).
function ensureSessionTypeAllowsTypingReverse(db) {
  const row = db.prepare(`SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'study_sessions'`).get();
  if (!row || row.sql.includes("'quiz_typing_reverse'")) return;

  db.exec(`
    ALTER TABLE study_sessions RENAME TO study_sessions_old;
    CREATE TABLE study_sessions (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      session_type TEXT NOT NULL CHECK (session_type IN ('study', 'quiz_choice', 'quiz_typing', 'quiz_matching', 'quiz_sentence', 'theory_drill', 'quiz_listening', 'level_exam', 'reading', 'media_exam', 'proficiency_test', 'theory_read', 'dialogue', 'quiz_typing_reverse')),
      started_at TEXT NOT NULL DEFAULT (datetime('now')),
      ended_at TEXT,
      cards_reviewed INTEGER NOT NULL DEFAULT 0,
      correct_count INTEGER,
...
export function runMigrations(db) {
  ...
  const migrate = db.transaction(() => {
    ...
    ensureSessionTypeAllowsTypingReverse(db);
